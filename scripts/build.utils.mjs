...
// Copyright 2022 (C) Abdul Habra. Version ${process.env.npm_package_version}.
...
export function copyIndexHtml() {
  ...
  html = html.replace('<script data-app></script>', '<script data-app src="out/helper-script.js"></script>')
...
export function build({format, minify, external, fileNameSuffix}) {
  const buildOptions = {
    entryPoints: ['src/Helper.js'],
    bundle: true,
    banner,
    outfile: `${out}/helper-${fileNameSuffix}.js`,
    minify: !!minify,
    format,
    external,
    globalName: 'jshelper'
